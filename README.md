...
**서버 측**:
소켓은 애플리케이션 계층에서 전송 계층(TCP/UDP)을 간편하게 쓸 수 있게 해주는 추상화 인터페이스입니다.

소켓이 있기에 애플리케이션 개발자가 socket()/bind()/connect()만 호출하면 OS가 TCP 3-way handshake, 패킷 재조립, 순서보장 등 복잡한 일을 알아서 처리해줍니다.
...
> **💡 [면접용 1분 요약 답변]**
> 
> "Spring에서 `@Transactional`을 붙이면 **CGLIB**이 원본 클래스를 상속받아서 프록시 클래스를 생성합니다. 이 프록시가 실제 빈 대신 스프링 컨테이너에 등록됩니다.
> 
> 프록시 내부에는 트랜잭션 시작, 커밋, 롤백 로직이 들어있습니다. 외부에서 메서드를 호출하면 프록시가 먼저 받아서 커넥션을 열고 트랜잭션을 시작한 뒤, 실제 객체의 메서드를 호출합니다. 메서드가 정상 종료되면 커밋하고, 예외가 발생하면 롤백합니다.
> 
> 이때 메서드 호출은 리플렉션이 아니라 **FastClass**라는 기법을 사용합니다. CGLIB이 프록시를 생성할 때 FastClass도 함께 만드는데, 이 클래스 안에는 각 메서드에 인덱스 번호가 부여되어 있습니다. 호출 시 switch문으로 인덱스를 분기해서 메서드를 직접 호출하기 때문에 리플렉션의 오버헤드가 없습니다.
> 
> 중요한 점은 프록시 기반이라 **같은 클래스 내에서 this로 다른 메서드를 호출하면 프록시를 거치지 않습니다.** 그래서 **내부 호출**에는 `@Transactional`이 적용되지 않습니다. 트랜잭션이 필요한 메서드는 반드시 외부에서 호출되어야 프록시가 가로채서 트랜잭션 처리를 할 수 있습니다."

<br>

> **[꼬리 질문] 왜 굳이 복잡하게 FastClass를 쓰나요?**
> 
> **Q1. 상속받은 거니까 그냥 프록시에서 `super.join()` 호출하면 안 되나요?**
> *   **안 됩니다.** 프록시 객체는 메서드 로직만 상속받았을 뿐, 내부 필드(`MemberRepository` 등)는 주입받지 못한 **깡통 객체**입니다.
> *   따라서 `super.join()`을 실행하면 `memberRepository`가 `null`이라서 **NPE(NullPointerException)**가 발생합니다. 그래서 필드가 채워진 **원본 객체(Target)**에게 일을 시켜야(위임) 합니다.
> 
> **Q2. 그럼 그냥 `target.join()` 하면 되지 않나요?**
> *   **리플렉션을 써야 합니다.** CGLIB 같은 라이브러리 입장에선 `target`이 무슨 클래스인지 알 수 없으므로(범용성), 메서드를 호출하려면 느린 **리플렉션**을 써야 합니다.
> *   이 **리플렉션 비용을 없애기 위해**, "1번은 join, 2번은 find" 처럼 메서드를 직접 호출해주는 코드(**FastClass**)를 미리 바이트코드로 만들어두고 **인덱스**로 빠르게 호출하는 것입니다.
...
<details>
<summary>무중단 배포 전략 (Rolling, Blue/Green, Canary)</summary>

<br>

**1. 롤링** (Rolling)
*   **방식**: 서버를 한 대씩 로드밸런서에서 제외시키고, 새 버전으로 교체하여 다시 투입하는 방식.
*   **예시**: 서버 A, B가 있을 때, A를 로드밸런서에서 제외(Traffic Off) -> A를 `v2`로 배포 -> A를 다시 투입 -> B도 동일하게 진행.
*   **장점**: 추가 자원이 거의 필요 없음.
*   **단점**: 배포 진행 중에는 가용 서버 수가 줄어들므로(Capacity 감소), 트래픽이 몰릴 때 부하가 커질 수 있음.

**2. 블루/그린** (Blue/Green)
*   **방식**: 구버전(Blue)과 똑같은 신버전(Green) 환경을 하나 더 만들고, 트래픽을 한 번에 Green으로 전환.
*   **예시**: Blue 그룹(서버 A, B)이 돌고 있을 때, 똑같은 스펙의 **Green 그룹**(서버 C, D)를 모두 띄운 뒤 -> 로드밸런서가 한 번에 Green 그룹(C, D)을 가리키도록 스위칭.
*   **장점**: 즉시 롤백 가능, 배포 속도 빠름.
*   **단점**: 서버 자원이 2배로 필요함 (비용 증가).

**3. 카나리** (Canary)
*   **방식**: 소수의 사용자(예: 5%)에게만 먼저 배포해보고, 문제없으면 전체 적용. (광산의 카나리아에서 유래)
*   **예시**: 전체 트래픽의 5%만 신버전 서버로 보내서 에러율 모니터링 후 점차 확대.
*   **장점**: 문제 발생 시 피해 최소화 (A/B 테스트 용이).
*   **단점**: 트래픽 제어 기술 필요, 구버전/신버전 공존.
